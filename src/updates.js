...
export async function setupUpdates({ hasDrafts, warn }) {
  ...
  try {
    ...
    let requested = false;
    const show = () => { button.hidden = !registration.waiting; };
    show();
    registration.addEventListener("updatefound", () => {
      registration.installing?.addEventListener("statechange", show);
    });
    button.onclick = () => {
      if (hasDrafts()) return warn("Save or explicitly cancel unfinished edits before updating.");
      requested = true;
    ...
    navigator.serviceWorker.addEventListener("message", event => {
      ...
      if (event.data?.type === "UPDATE_READY" && requested) {
        if (hasDrafts()) warn("Update is ready. Save your edits, then reopen this window.");
        else location.reload();
      }
    });
    document.addEventListener("visibilitychange", () => {
      if (!document.hidden) registration.update().then(show).catch(() => {});
    });
  } catch { warn("Offline support could not start. Your database is separate from the offline cache."); }
}
