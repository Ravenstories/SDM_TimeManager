...
export function inputTimestamp(value, original) {
  ...
  const canonical = text => {
    ...
    return match ? match[1] + ":" + (match[2] || "00") + "." + (match[3] || "").padEnd(3,"0") : null;
  };
  if (original !== undefined && canonical(value) === canonical(localInputValue(original))) return original;
  const timestamp = new Date(value).getTime();
  if (!Number.isFinite(timestamp) || canonical(value) !== canonical(localInputValue(timestamp)))
    throw new Error("Choose a valid local date and time. This time may not exist at a daylight-saving transition.");
  return timestamp;
}
