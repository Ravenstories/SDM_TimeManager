...
function unique(items, field = "id") {
  return new Set(items.map((x) => x[field])).size === items.length;
}
export function validateState(s) {
  const invalid = () => {
    throw Error("This is not a valid SDM backup or record.");
  };
  if (
    !s ||
    s.version !== 2 ||
  ...
  if (
    ...
    new Set(s.responsibilities.map((r) => r.name.toLowerCase())).size !==
  ...
  if (
    ...
    (!s.active ||
      !validId(s.active.id) ||
      !known(s.active.responsibilityId) ||
      !validTime(s.active.start) ||
      s.sessions.some((e) => e.id === s.active.id))
  )
    invalid();
  if (
    !s.preferences ||
    !["clock", "decimal"].includes(s.preferences.timeFormat) ||
    typeof s.preferences.compact !== "boolean"
  )
    invalid();
  return s;
}
export function migrateState(input) {
  ...
  const old = validateLegacy(input),
    next = emptyState();
  const extraUsed =
    old.sessions.some((e) => e.role === "extra") ||
    old.notes.some((n) => n.role === "extra") ||
    old.active?.role === "extra";
  if (old.extraClock || extraUsed) {
    let name = old.extraClock?.name.trim() || "Legacy additional";
    if (
      next.responsibilities.some(
        (r) => r.name.toLowerCase() === name.toLowerCase(),
      )
    )
      name = name.slice(0, 28) + " (additional)";
    next.responsibilities.push({
      id: "extra",
      name,
      classification: old.extraClock
        ? old.extraClock.countsAsWork
          ? "work"
          : "nonwork"
        : "unresolved",
      archived: !old.extraClock,
    });
  }
  const convert = (item) => {
    const { role, ...rest } = item;
    return { ...rest, responsibilityId: role };
  };
...
export function responsibility(state, id) {
  ...
  if (!r)
    throw Error("This responsibility no longer exists. Reload the record.");
  return r;
}
export function switchRole(state, id, now, sessionId) {
  ...
  if (id !== null) {
    ...
    if (r.archived || r.classification === "unresolved")
      throw Error(
        "Restore and classify this responsibility before starting it.",
      );
  }
  ...
  next.active =
    id === null ? null : { id: sessionId, responsibilityId: id, start };
  return validateState(next);
}
export function saveSession(state, entry, now = Date.now(), original) {
  ...
  if (original) {
    assertUnchanged(
      state.sessions.find((e) => e.id === original.id),
      original,
    );
    if (
      ["id", "responsibilityId", "start", "end"].every(
        (key) => entry[key] === original[key],
      )
    )
      return state;
  }
  ...
  if (entry.end > now)
    throw Error("Completed entries cannot end in the future.");
  if (original)
    assertUnchanged(
      state.sessions.find((e) => e.id === original.id),
      original,
    );
  else if (state.sessions.some((e) => e.id === entry.id))
    throw Error("This entry already exists.");
  const other = state.sessions.filter((e) => e.id !== entry.id);
  const occupied = state.active
    ? [...other, { ...state.active, end: Infinity }]
    : other;
  if (occupied.some((e) => entry.start < e.end && entry.end > e.start))
    throw Error("This entry overlaps another tracked session.");
  return validateState({
    ...state,
    sessions: [...other, { ...entry }].sort((a, b) => a.start - b.start),
  });
}
export function removeSession(state, original) {
  ...
  return {
    ...state,
    sessions: state.sessions.filter((e) => e.id !== original.id),
  };
}
export function stopActive(state, end, now, original) {
  assertUnchanged(state.active, original);
  if (!state.active || !validTime(end) || end < state.active.start || end > now)
    throw Error("Choose a stop time between the start and now.");
  const entry = { ...state.active, end };
  const next = { ...state, active: null };
  if (end === entry.start)
    return { ...next, sessions: [...next.sessions, entry] };
...
export function saveNote(state, note, original) {
  ...
  if (original)
    assertUnchanged(
      state.notes.find((n) => n.id === original.id),
      original,
    );
  else if (state.notes.some((n) => n.id === note.id))
...
export function saveResponsibility(state, record, original) {
  ...
  if (
    next.classification === "unresolved" &&
    original?.classification !== "unresolved"
  )
    throw Error("Choose work or non-work.");
  if (original)
    assertUnchanged(
      state.responsibilities.find((r) => r.id === original.id),
      original,
    );
  else if (state.responsibilities.some((r) => r.id === next.id))
    throw Error("This responsibility already exists.");
  if (
    state.responsibilities.some(
      (r) =>
        r.id !== next.id && r.name.toLowerCase() === next.name.toLowerCase(),
    )
  )
    throw Error("Use a unique responsibility name.");
  const responsibilities = original
    ? state.responsibilities.map((r) => (r.id === next.id ? next : r))
...
export function archiveResponsibility(state, original, archived, now) {
  ...
  if (archived && state.active?.responsibilityId === original.id)
    next = stopActive(
      state,
      Math.max(now, state.active.start),
      Math.max(now, state.active.start),
      state.active,
    );
...
export function moveResponsibility(state, id, delta) {
  const items = [...state.responsibilities],
    index = items.findIndex((r) => r.id === id),
    target = index + delta;
...
export function totals(state, date, now) {
  ...
  const byResponsibility = Object.fromEntries(
    state.responsibilities.map((r) => [r.id, 0]),
  );
  for (const e of state.active
    ? [...state.sessions, { ...state.active, end: now }]
    : state.sessions)
    byResponsibility[e.responsibilityId] += Math.max(
      0,
      Math.min(e.end, end) - Math.max(e.start, start),
    );
...
export function responsibilityImpact(state, edited, now) {
  ...
  const entries = state.sessions.filter(
    (e) => e.responsibilityId === edited.id,
  );
  const elapsed =
    entries.reduce((sum, e) => sum + e.end - e.start, 0) +
    (state.active?.responsibilityId === edited.id
      ? Math.max(0, now - state.active.start)
      : 0);
  return {
    entries: entries.length,
    notes: state.notes.filter((n) => n.responsibilityId === edited.id).length,
    workDelta:
      elapsed *
      (Number(edited.classification === "work") -
        Number(original.classification === "work")),
  };
}
export function needsReview(active, now) {
  return Boolean(
    active &&
      (localDate(active.start) !== localDate(now) ||
        now - active.start >= 12 * 3600000),
  );
}
